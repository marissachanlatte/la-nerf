...
@dataclass
class LaSDFFieldConfig(FieldConfig):
    """SDF Model Config"""

    _target: Type = field(default_factory=lambda: LaSDFField)
    num_layers: int = 8
    ...
    """which laplace backend to use"""
    laplace_method: Literal["laplace", "linearized-laplace"]="laplace"
    """which laplace method to use"""
    laplace_num_samples: int = 10
    """number of samples to use for laplace"""
    laplace_hessian_shape: Literal["diag", "kron", "full"]="diag"
    """shape of the hessian to use for laplace"""



class LaSDFField(Field):
    """_summary_
    ...
    def __init__(
        ...
        if laplace_backend == "nnj":
            ...
            self.geo_mlp = nnj.utils.convert_to_nnj(self.geo_mlp)
        ...
        self.geo_hessian = 10**6 * torch.ones_like(
            parameters_to_vector(self.geo_mlp.parameters()),
            device="cuda:0"
        )

        # initalize hessian calculator and sampler
        self.hessian_calculator = pytorch_laplace.MSEHessianCalculator(
            hessian_shape=self.laplace_hessian_shape,
        ...
        if not self.training:
            ...
            if self.laplace_method == "laplace":
                ...
                if self.resample_density_parameters:
                    sigma_q = self.la_sampler.posterior_scale(hessian=self.geo_hessian)
                    mu_q = parameters_to_vector(self.geo_mlp.parameters())
                ...
                density_mu, density_sigma = self.la_sampler.normal_from_samples(x=sdf, samples=self.density_weight_samples, model=self.laplace_density)

            elif self.laplace_method == "linearized-laplace":
                density_mu, density_sigma = self.la_sampler.linearized_laplace(
                    x=sdf,
                    model=self.laplace_density,
                    hessian=self.geo_hessian,
                )
            else:
            ...
            self.geo_hessian = 0.999 * self.geo_hessian + hessian_batch
        return density, sdf, density_mu, density_sigma
    ...
    def get_colors(self, points, directions, gradients, geo_features, camera_indices):
        ...
        if not self.training:
            ...
            if self.laplace_method == "laplace":
                ...
                rgb_out['rgb_mu'] = rgb_mu
                rgb_out['rgb_sigma'] = rgb_sigma
    ...
    def get_outputs(self, ray_samples: RaySamples, return_alphas=False, return_occupancy=False):
        ...
        rgb_out = self.get_colors(inputs, directions_flat, gradients, geo_feature, camera_indices)

        density = self.laplace_density(sdf)

        rgb = rgb_out['rgb'].view(*ray_samples.frustums.directions.shape[:-1], -1)
        ...
        if not self.training:
            outputs.update({"rgb_sigma": rgb_out['rgb_sigma'].view(*ray_samples.frustums.directions.shape[:-1], -1)})
            outputs.update({"rgb_mu": rgb_out['rgb_mu'].view(*ray_samples.frustums.directions.shape[:-1], -1)})
    ...
    def forward(self, ray_samples: RaySamples, return_alphas=False, return_occupancy=False):
        ...
        if density_sigma is not None:
            field_outputs["density_sigma"] = density_sigma 
